...
type Repository interface {
    ...
    GetTuple(namespace, object, relation, userNamespace, uid, relation string) (model.TupleRecord, error)
    ...
    GetTuplesFromObjRel(namespace, objectId, relation string) ([]model.TupleRecord, error)

    // Return tuples where their users are specified by the given userset
    // aka reverse lookup
    // Does not perform userset chasing
    GetTuplesFromUserset(namespace, id, relation string) ([]model.TupleRecord, error)

    // Purge tuple from storage. 
    RemoveTuple(tuple Tuple) error

    GetNamespace(namespace string) (model.Namespace, error)

    SetNamespace(namespace mode.Namespace) error
